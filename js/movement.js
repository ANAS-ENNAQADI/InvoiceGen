...
async function generatePDF() {
  try {
   ...
   spanIcon.style.display="none";
    ...
    const pdfWidth = canvas.width / 2; // You can adjust this value to set the image width in the PDF
  ...
  $("#submitInvoice").click(function() {
      ...
      $.ajax({
          ...
          success: function(response) {
              // Do nothing here, no messages or actions needed
          },
          error: function(xhr, status, error) {
              // Handle errors if necessary, but no messages will be shown
          }
      });
  });
});
